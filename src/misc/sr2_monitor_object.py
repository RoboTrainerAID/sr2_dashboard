...
# Description: Infrastructure for executing external processes,
...
class ProcStatus():
    ...
    # The process has failed to exit properly (exit code != 0) or the clean up
    # of the files associated with the process has failed
    FAILED_STOP = 4
...
class SR2Worker(QObject):
    ...
    def __init__(self, name, cmd, pkg, args=None, parent=None):
        ...
        if not cmd:
            return
        if not cmd and not pkg and not args:
            return
        ...
        self.dir_name = '/tmp'
        self.path = self.dir_name + '/pid_' + cmd + \
            (pkg if pkg else '') + (args if args else '') + name
...
#      self.cleanup()
    ...
    def recover(self):
        ...
        if self.pid:
            rospy.logwarn(
                'SR2: Process with PID %d already running. Will connect to it', self.pid)
            self.active = True
    ...
    def start(self):
        ...
        if not self.active and not self.pid:
            self.active, self.pid = QProcess.startDetached(
                self.cmd, self.args, self.dir_name)
            QThread.sleep(5)
    ...
    def stop(self):
        ...
        if self.active and self.pid:
            try:
                ...
                rospy.logerr(
                    'SR2: Sending SIGINT to given PID %d failed', self.pid)
...
#    print(' --- worker thread ID: %d ---' % QThread.currentThreadId())
        if self.active and self.pid:
            ...
            if not running:
                # This case occurs if the started external process has crashed
                # or was stopped with other mean outside the dashboard
                self.setStatus(ProcStatus.FAILED_STOP)
                self.cleanup()
                self.active = False
                self.pid = None
    ...
    statusProp = pyqtProperty(
        int, fget=getStatus, fset=setStatus, notify=statusChanged_signal)
    ...
    def checkPidFileExists(self):
        ...
        try:
            if not path.isfile(self.path):
                ...
                rospy.loginfo(
                    'SR2: File %s was successfully created', self.path)
                return False
            else:
                return True
        except IOError:
            rospy.logerr(
                'SR2: Unable to write PID file "%s". Maybe you don\'t have the permissions to write to the given path?', self.path)
            print(self.path)
            return False
    ...
    def writePidToFile(self):
        ...
        '''
        rospy.loginfo('SR2: Attempting to write %s to %s' %
                      (str(self.pid), self.path))
        if not self.pid:
            return
        # Writing to a non-existent file automatically creates a new one
        # however we also want to check if it's empty or not!
        if self.checkPidFileExists() and self.isPidFileEmpty():
            rospy.loginfo(
                'SR2: File exists and is not empty. Will write to file')
    ...
    def isPidFileEmpty(self):
        ...
        '''
        # Use of exception handling here is necessary because if file doesn't
        # exist the following won't work
        try:
        ...
        if self.checkPidFileExists() and not self.isPidFileEmpty():
            with open(self.path, 'r') as pidFile:
                ...
                if not pid:
                    ...
                    if not pid_running:
                        # Delete PID file if the contained PID is invalid
                        # (represents a process that isn't currently running)
                        remove(self.path)
                        return None
                    else:
                        return pid
    ...
    def cleanup(self):
        ...
        try:
            if self.checkPidFileExists():
                ...
                rospy.loginfo('SR2: No PID file found. Nothing to cleanup')
        except:
            rospy.logerr('SR2: Failed to delete PID file')
